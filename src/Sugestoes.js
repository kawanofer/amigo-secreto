...
const Sugestoes = () => {
      ...
      <ul>
        {names?.map((name) => {
          ...
          return (
            <li key={name}>
              <a
                href={`#${upperSnakeCaseName}`}
                onClick={() => handleLinkClick(upperSnakeCaseName)}>
  ...
  const CreateGiftForm = () => {
    return names?.map((name) => {
      ...
      return (
        <section
          id={upperSnakeCaseName}
          key={name}
          ref={sectionRefs.current[upperSnakeCaseName]}>
          <h2>{name}</h2>
          <form onSubmit={handleSubmit((data) => onSubmit(data))}>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              <textarea
                placeholder='Digite aqui sua sugestão de presente'
                {...register(`${upperSnakeCaseName}`)}
              />
  ...
  return (
    <div className='container'>
      <div className='header'>{CreateSideMenu()}</div>
      <div className='main'>
        {CreateHeader()}
        {CreateGiftForm()}
      </div>
